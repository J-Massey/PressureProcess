# Processed freestream pressure (NC -> nkd) — phase2 variant.
...
def save_prod_fs_pressure(
    *,
    ...
    with h5py.File(fs_prod, "w") as hf:
        hf.attrs["title"] = "Freestream pressure (nose-cone) - production [phase2]"
        hf.attrs["fs_Hz"] = FS
        ...
        hf.attrs["description"] = (
            "Freestream NC signals (phase2), corrected with NC -> nkd FRF "
            "from semi-anechoic calibration."
        )
