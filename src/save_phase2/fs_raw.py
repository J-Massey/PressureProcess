# Freestream raw pressure (NC channel) — phase2 variant.
...
def save_raw_fs_pressure(
    *,
    ...
    with h5py.File(fs_raw, "w") as hf:
        hf.attrs["title"] = r"Freestream pressure (nose-cone) - raw \& calibration [phase2]"
        hf.attrs["fs_Hz"] = FS
        ...
        hf.attrs["description"] = (
            "Raw freestream NC signals (phase2, close spacing only). "
            "Pressures (0, 50, 100) psig. Includes the simultaneous "
            "semi-anechoic NC calibration."
        )
