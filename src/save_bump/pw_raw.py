# Wall-pressure raw (pinhole) — bump variant.
import os
...
def save_raw_ph_pressure(
    *,
    ...
    with h5py.File(ph_raw, "w") as hf:
        hf.attrs["title"] = r"Wall-pressure (pin-hole) - raw \& calibration [bump]"
        hf.attrs["fs_Hz"] = FS
        ...
        hf.attrs["description"] = (
            "Raw wall-pressure signals (bump, rough surface). Pressures "
            "(0, 50, 100) psig with close+far pinhole spacings. Includes "
            "PH<->NC semi-anechoic calibration."
        )
