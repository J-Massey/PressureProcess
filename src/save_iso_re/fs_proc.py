# Processed freestream pressure (NC -> nkd) — iso_re variant.
# iso_re has no matching NC semi-anechoic calibration runs at (0, 30, 50) psig,
# so the NC -> nkd FRF is the identity by design.
...
def save_prod_fs_pressure(
    *,
    ...
    with h5py.File(fs_prod, "w") as hf:
        hf.attrs["title"] = "Freestream pressure (nose-cone) - production [iso_re]"
        hf.attrs["fs_Hz"] = FS
        ...
        hf.attrs["description"] = (
            "Freestream NC signals (iso_re), identity FRF applied (no matching "
            "NC semi-anechoic calibration for (0, 30, 50) psig)."
        )

        g_fs = hf.create_group("freestream_production")

        # Identity FRF: NC = nkd. Pinned here, NOT a fallback.
        f_cal_nkd = np.array([0.0, FS / 2.0], dtype=float)
        H_fused_nkd = np.array([1.0 + 0.0j, 1.0 + 0.0j], dtype=complex)
        nc_note = "Identity FRF (iso_re has no matching NC semi-anechoic calib)"
