# Processed wall-pressure (pinhole, FRF-corrected + noise-cancelled) — iso_re variant.
# iso_re uses identity FRF for the NC -> nkd stage (no matching semi-anechoic
# calibration at (0, 30, 50) psig).
...
def _resolve_noise_canceller(signal_len: int) -> str:
    ...
    if method in {"hybrid", "wiener"}:
        return method
    raise ValueError("PW_NOISE_CANCELLER must be one of: auto, wiener, hybrid")
...
def save_corrected_pressure(
    *,
    ...
    # iso_re: identity FRF for NC -> nkd (pinned, not a fallback).
    ...
    with h5py.File(ph_processed, "w") as hf:
        hf.attrs["title"] = "Wall-pressure (pin-hole) - production [iso_re]"
        hf.attrs["fs_Hz"] = FS
        ...
        hf.attrs["description"] = (
            "Processed wall-pressure (iso_re). PH -> NC FRF applied from "
            "semi-anechoic calibration; NC -> nkd uses identity FRF."
        )
        ...
        for i, L in enumerate(labels):
            ...
            with h5py.File(ph_raw, "r") as f_raw, h5py.File(nkd_raw, "r") as f_nkd:
                ...
                with h5py.File(f"{cal_base}/PH/calibs_{int(psigs[i])}.h5", "r") as hf_cal:
                    f_cal = np.asarray(hf_cal["frequencies"][:], dtype=WORK_DTYPE)
                    H_cal = np.asarray(hf_cal["H_fused"][:], dtype=np.complex64)
