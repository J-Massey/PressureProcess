# Wall-pressure raw (pinhole) — iso_re variant.
import os
...
def save_raw_ph_pressure(
    *,
    ...
    with h5py.File(ph_raw, "w") as hf:
        hf.attrs["title"] = r"Wall-pressure (pin-hole) - raw \& calibration [iso_re]"
        hf.attrs["fs_Hz"] = FS
        ...
        hf.attrs["description"] = (
            "Raw wall-pressure signals (iso_re). Pressures (0, 30, 50) psig with "
            "only 'close' pinhole spacing. Includes PH<->NC semi-anechoic "
            "calibration runs."
        )
        ...
        for i, L in enumerate(labels):
            ...
            # PH<->NC semi-anechoic calibration (per pressure, two positions).
