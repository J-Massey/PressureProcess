"""
Production freestream plots for the phase2 pipeline.

Per pressure: PSDs of the freestream NC channel BEFORE (raw, dashed) and AFTER
(production, solid) the NC->nkd FRF correction from fs_proc. Spacings overlaid.
"""
    ...
    with h5py.File(cfg.NKD_RAW_FILE, "r") as hf_raw, \
        ...
        for label in labels:
            ...
            for spacing in cfg.SPACINGS:
                color = SPACING_COLORS.get(spacing, "#444444")
            ...
            ax.set_title(f"phase2 {label} -- freestream NC raw (dashed) vs production (solid)")
            ax.set_xlabel("$f$ [Hz]")
