"""
Noise-rejection diagnostic via coherence for the phase2 pipeline.
...
drive the canceller. We plot per pressure x spacing x channel:

  - gamma^2(NC_pre, PH_frf_corrected_pre)   -- pre-rejection  (dashed)
  - gamma^2(NC_pre, PH_noise_rejected)      -- post-rejection (solid)

Both the NC reference and the pre-rejection PH signal are demeaned and
bandpass-filtered (1 Hz HP, analog LP) before coherence is computed, to
mirror pw_proc's exact pre-Wiener preprocessing.
"""
...
def plot_coherence() -> None:
    ...
    with h5py.File(cfg.PH_PROCESSED_FILE, "r") as hf_w, \
        ...
        for label in g_w:
            ...
            fig.suptitle(rf"phase2 {label} -- NC$\leftrightarrow$PH coherence (pre vs post Wiener)",
                         fontsize=10)
