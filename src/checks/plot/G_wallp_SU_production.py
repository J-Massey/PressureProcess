...
def _local_u_tau_re_tau(g_corr, sp, channel_key, gL):
    """Per-(spacing, channel) u_tau/Re_tau from dataset attrs when present
    (pw_proc may stamp them), otherwise fall back to the per-label values."""
    try:
...
def plot_stages_3panel(
    *,
    ...
    and all 3 pipeline stages (raw, FRF-corrected, Wiener-rejected) for
    phase1. Matches plot_bump.production.plot_stages_3panel exactly.

    Smoothing (cosmetic, applied to the Wiener-rejected stage only): PSD is
    ...
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf_raw, \
            ...
            for sp, ch, _lbl, col in POSITION_TRACES:
                ...
                def _draw_stage(group, linestyle: str, lw: float, alpha: float):
                    if group is None or sp not in group or ch not in group[sp]:
                        return
                    f_raw, p_raw = compute_spec(group[f"{sp}/{ch}"][:])
                    if group is g_rej:
        ...
        out_dir = FIG_DIR / "production"
        out_dir.mkdir(parents=True, exist_ok=True)
        out = out_dir / "G_wallp_SU_production_stages.png"
