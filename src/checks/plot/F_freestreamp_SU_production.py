...
def plot_fs_raw():
    with h5py.File(cfg.NKD_PROCESSED_FILE, "r") as f_raw:
        g_root = f_raw["freestream_production"]
        labels = list(g_root.keys())
        if not labels:
            raise KeyError("No labels found in freestream_production")
        ...
        if not spacing_order:
            raise KeyError("No spacing groups found in freestream_production")
    ...
    for ax in axes:
        ax.legend()
    plt.savefig(FIG_DIR / "F_freestreamp_SU_production.png", dpi=600)
