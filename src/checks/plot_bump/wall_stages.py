"""
Wall-pressure end-to-end stage overlay for the bump pipeline.

For each pressure, a 2x2 grid (rows = PH1/PH2, cols = close/far) showing the
three pw_proc stages on one axis per channel/spacing:
  1. raw          -- straight from pw_raw (wallp_raw)
  2. frf_corrected-- after PH->NC then NC->nkd FRFs (wallp_production/.../frf_corrected_signals)
  3. noise_rejected -- after detrend + bandpass + Wiener cancellation (.../fs_noise_rejected_signals)

Lets you confirm each stage of pw_proc independently rather than only seeing
the dashed pre/solid post overlay in production.plot_noise_rejected.
"""
...
def plot_wall_stages() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf_raw, \
        ...
        for label in labels:
            ...
            fig.suptitle(f"bump {label} -- wall pipeline stages", fontsize=10)
