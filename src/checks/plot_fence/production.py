"""
Inner-normalised production plot for the fence pipeline.

A single 3-panel figure (0/50/100 psig, sharey) showing the available sensor
positions and the three pipeline stages (raw, FRF-corrected, Wiener-rejected)
overlaid on inner-scaled axes. Each line is normalised by its own position-
specific u_tau (read from the per-channel dataset attrs stamped by
save_fence/pw_proc.py).

Mirrors plot_bump.production exactly. Fence's SPACINGS=("combined",) so the
position legend resolves to P1=(combined, PH1) and P2=(combined, PH2) only.
"""
...
# Fence now uses bump's (close, far) layout: four positions per panel.
POSITION_TRACES = (
...
def _smooth_psd_logf(
    ...
    *,
    ...
    """Resample a one-sided PSD onto a log-f grid (ppo points-per-octave) and
    smooth with a uniform kernel of width span_oct. Cosmetic only -- applied
    just before inner-scaling on the Wiener-rejected stage."""
...
def plot_stages_3panel(
    *,
    ...
    and the three pipeline stages -- exact mirror of
    plot_bump.production.plot_stages_3panel. Fence has SPACINGS=("combined",),
    so the position legend filters down to P1/P2 only.

    Smoothing (cosmetic, applied to Wiener-rejected stage only): PSD is
    resampled onto a `smooth_ppo`-ppo log-f grid and convolved with a
    `smooth_span_oct`-octave uniform kernel before inner-scaling.
    """
