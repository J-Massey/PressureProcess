...
from scipy.signal import get_window, welch, coherence
...
def compute_coherence(x: np.ndarray, y: np.ndarray, fs: float = FS, nperseg: int = NPERSEG):
    """Welch coherence with consistent settings. Returns f [Hz], Cxy."""
    x = np.asarray(x, dtype=float)
...
def plot_wall_hz_pa() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        ...
        for label in labels[:1]:
            ...
            ax.set_xlabel("T [s]")
            ax.set_ylabel(r"$\sqrt{\phi_{pp}}$ [Pa/Hz]")
...
def plot_wall_plus_loglin_raw() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        g_root = hf["wallp_raw"]
        fs = float(hf.attrs.get("fs_Hz", FS))
        labels = _ordered_labels(list(g_root.keys()))
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$({f \phi_{pp}}^+)_{\mathrm{wall}}$")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_plus_loglin_raw.png", dpi=600)
            plt.close(fig)

def plot_wall_plus_loglog_raw() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        g_root = hf["wallp_raw"]
        fs = float(hf.attrs.get("fs_Hz", FS))
        labels = _ordered_labels(list(g_root.keys()))
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$({f \phi_{pp}}^+)_{\mathrm{wall}}$")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_plus_loglog_raw.png", dpi=600)
            plt.close(fig)


def plot_wall_out_loglin_raw() -> None:
    """
    Outer-scale wall pressure 
    """
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$({f \phi_{pp}}^+)_{\mathrm{wall}}$")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_out_loglin_raw.png", dpi=600)
            plt.close(fig)

def plot_wall_out_loglog_raw() -> None:
    """
    Outer-scale wall pressure 
    """
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$({f \phi_{pp}}^+)_{\mathrm{wall}}$")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_out_loglog_raw.png", dpi=600)
            plt.close(fig)



def plot_wall_bump_loglin_raw() -> None:
    """
    Outer-scale wall pressure 
    """
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$({f \phi_{pp}}^+)_{\mathrm{wall}}$")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_bump_loglin_raw.png", dpi=600)
            plt.close(fig)

def plot_wall_bump_loglog_raw() -> None:
    """
    Outer-scale wall pressure 
    """
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$({f \phi_{pp}}^+)_{\mathrm{wall}}$")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_bump_loglog_raw.png", dpi=600)
            plt.close(fig)

# Now we plot the raw free-stream in loglin, then the coherence
...
def plot_wall_raw_hz_pa() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf:
        ...
        for label in labels[:1]:
            ...
            ax.set_title(f"{label} wall raw")
            ax.set_xlabel("T [s]")
            ax.set_ylabel(r"$\sqrt{\phi_{pp}}$ [$Pa^2$/Hz]")
            ax.legend(fontsize=8)
            fig.savefig(DIAG_DIR / f"{label}_wall_raw_hz_pa.png", dpi=300)
            plt.close(fig)

def plot_wall_fs_coh_loglin() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf, h5py.File(cfg.NKD_PROCESSED_FILE, "r") as hf_proc:
        g_root = hf["wallp_raw"]
        g_root_fs = hf_proc["freestream_production"]
        fs = float(hf.attrs.get("fs_Hz", FS))
        labels = _ordered_labels(list(g_root.keys()))
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            for i2, spacing in enumerate(SPACING_ORDER):
                for i3, channel in enumerate(WALL_CHANNELS):
                    # f, pxx = compute_spec(g_label[spacing][channel][:], fs=fs)
                    # f, pxx_fs = compute_spec(g_label_fs[spacing][channel][:], fs=fs)
                    f, coh = compute_coherence(g_label[spacing][channel][:], g_label_fs[spacing]["NC_Pa"][:], fs=fs)
                    mask = f > 0.0
                    delta = deltas[i1][i2][i3]
                    Ue = Ues[i1][i2][i3]
            ...
            ax.grid(True, which="major", linestyle="--", linewidth=0.4, alpha=0.7)
            ax.set_title(f"{label} wall freestream coherence")
            ax.set_xlabel(r"$T^h$")
            ax.set_ylabel(r"$\sqrt{\phi_{pp}}$ [$Pa^2$/Hz]")
            ax.legend(fontsize=8, loc="upper left")
            fig.savefig(DIAG_DIR / f"{label}_wall_fs_coh_loglin.png", dpi=600)
            plt.close(fig)


def plot_wall_fs_coh_loglog() -> None:
    with h5py.File(cfg.PH_RAW_FILE, "r") as hf, h5py.File(cfg.NKD_PROCESSED_FILE, "r") as hf_proc:
        g_root = hf["wallp_raw"]
        g_root_fs = hf_proc["freestream_production"]
        fs = float(hf.attrs.get("fs_Hz", FS))
        ...
        for i1, label in enumerate(labels[:1]):
            ...
            ax.set_ylabel(r"$\sqrt{\phi_{pp}}$ [$Pa^2$/Hz]")
            ax.legend(fontsize=8, loc="upper left")
            fig.savefig(DIAG_DIR / f"{label}_wall_fs_coh_loglog.png", dpi=600)
            plt.close(fig)
...
def plot_cleaned_by_case() -> None:
    ...
    # plot_wall_plus_loglin_raw()
    # plot_wall_plus_loglog_raw()
    # plot_wall_out_loglin_raw()
    # plot_wall_out_loglog_raw()
    # plot_wall_bump_loglin_raw()
    # plot_wall_bump_loglog_raw()
    # plot_wall_raw_hz_pa()
    plot_wall_fs_coh_loglin()
    plot_wall_fs_coh_loglog()
