"""
Calibration plots for the fence pipeline (same shape as bump).
"""
...
def plot_ph_calibration() -> None:
    ...
    for p_si in pressures:
        ...
        for f, H, label, color, ls in (
            (f1, H1, r"PH1$\to$NC (applied to PH1)", "#1e8ad8", "-"),
            (f2, H2, r"PH2$\to$NC (applied to PH2)", "#ff7f0e", "-"),
            (ff, Hf, "fused (diagnostic only)", "#777777", "--"),
        ):
            mask = np.asarray(f) > 0.0
            if not np.any(mask):
                continue
            mag = _mag_db(np.asarray(H)[mask])
            ax_mag.semilogx(np.asarray(f)[mask], mag,
                            color=color, linewidth=1.0, label=label, linestyle=ls)
            mag_vals.append(mag[np.isfinite(mag) & (mag > -100.0)])

        for f, g, label, color, ls in (
            (f1, g1, r"$\gamma^2_{PH1,NC}$", "#1e8ad8", "-"),
            (f2, g2, r"$\gamma^2_{PH2,NC}$", "#ff7f0e", "-"),
            (ff, gf, r"$\gamma^2$ fused", "#777777", "--"),
        ):
            mask = np.asarray(f) > 0.0
            ax_coh.semilogx(np.asarray(f)[mask], np.asarray(g)[mask],
                            color=color, linewidth=1.0, label=label, linestyle=ls)

        ax_mag.set_title(f"PH calibration -- fence {p_si} psig")
...
def plot_nc_calibration() -> None:
    ...
    for p_si in pressures:
        ...
        fig, (ax_mag, ax_coh) = plt.subplots(2, 1, figsize=(6.0, 4.8), tight_layout=True)
        ax_mag.semilogx(f[mask], _mag_db(H[mask]), color="#26bd26", linewidth=1.0, label=r"NC$\to$nkd")
        ax_coh.semilogx(f[mask], g[mask], color="#26bd26", linewidth=1.0, label=r"$\gamma^2$ NC,nkd")

        ax_mag.set_title(f"NC calibration -- fence {p_si} psig")
        ax_mag.set_ylabel(r"$|H(f)|$ [dB]")
        ax_mag.grid(True, which="major", linestyle="--", linewidth=0.4, alpha=0.7)
        ax_mag.legend(fontsize=8)
        ax_coh.set_xlabel("$f$ [Hz]")
