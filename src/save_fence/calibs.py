# Per-pressure physical FRFs (PH to NC) from semi-anechoic runs — fence variant.
...
def _save_ph_tf_plot(
    *,
    ...
    ax_mag.set_title(f"PH Calibration TF Check ({p_si} psig) [fence]")
...
def save_PH_calibs(
    *,
    ...
    for p_si, fcut in zip(pressures, cfg.F_CUTS):
        psig = float(p_si)
        m1_path = base / f"calib_{p_si}psig_1.mat"
        m2_path = base / f"calib_{p_si}psig_2.mat"
        if not (m1_path.exists() and m2_path.exists()):
            print(f"[skip] missing PH calib files for {p_si} psig at {base}")
            continue

        m1 = loadmat(m1_path)
        ph1_v, _, nc_v, *_ = _extract_channel_data(m1).T
        ph1_pa = correct_pressure_sensitivity(volts_to_pa(ph1_v, "PH1"), psig)
        nc1_pa = correct_pressure_sensitivity(volts_to_pa(nc_v, "NC"), psig)
        ...
        ph2_pa = correct_pressure_sensitivity(volts_to_pa(ph2_v, "PH2"), psig)
        nc2_pa = correct_pressure_sensitivity(volts_to_pa(nc_v2, "NC"), psig)
        ...
        with h5py.File(out, "w") as hf:
            # Per-run grids (welch grid from estimate_frf) — these are what
            # H1/H2 live on. f1 == f2 when fs and nperseg match.
            ...
            # Fused grid + fused TF kept as a diagnostic backup; fence's
            # pw_proc applies H1 to PH1 and H2 to PH2 (per-channel) instead.
            ...
            hf.attrs["application"] = "per-channel: apply H1 to PH1, H2 to PH2"
...
def save_NC_calibs(
    *,
    ...
    for p_si, fcut in zip(pressures, f_cuts):
        psig = float(p_si)
        m_path = base / f"{p_si}psig/nkd-ns_nofacilitynoise.mat"
        if not m_path.exists():
            print(f"[skip] missing NC calib file: {m_path}")
            continue
        m1 = loadmat(m_path)
        ic(m1.keys())
        if p_si == 100:
